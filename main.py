...
async def setup_step2(message: types.Message):
    ...
    await message.answer(f"✅ Трекер запущен! Теперь кнопки меню всегда внизу.", reply_markup=main_menu())
...
async def show_stats(message: types.Message):
    ...
    await message.answer(f"🏆 *Твой успех:*\n\n"
                         f"⏳ Ты держишься: *{diff.days}д {diff.seconds//3600}ч*\n"
                         f"💰 Сэкономлено: *{round(money, 2)} руб.*", parse_mode="Markdown")
